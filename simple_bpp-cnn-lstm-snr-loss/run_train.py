...
def run_train():
    ...
    fold = 0
    for fold in [0,2,3,4]:#,1,2,3,4  #0, 1,2,3,4
        out_dir = '/sn1-fold-%d'%fold
        initial_checkpoint = \
        ...
        train_index, _ = filter_by_signal_to_noise(train_index, df, 2)

        print('train_index:', train_index[:10], 'valid_index:', valid_index[:10],)
        if 1: #check overlap
            print(set(train_index).intersection(set(valid_index)))

        train_dataset = RNADataset(df.loc[train_index].reset_index(drop=True),train_augment)
